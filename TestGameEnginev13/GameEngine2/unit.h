...
public:
	...
	short transferring; //!< This stores transfer state. 0=not transfering resources, 1=going to building to get resources to transfer, 2=going to building to deposit resources to transfer
	short transferfrom; //!< which building its transfering from
	short transferto; //!<which building its transfering to
	unsigned short message; //!< A message that can be sent by a different unit telling this unit to move out of the way. May be upgraded to int later if I come up with more messages to send
	int adjusting; //!< if its adjusting to a units message
	...
	void train();
	void trainlieut();
	int chkmvp1(tile &checkwhat, bool checkelev=true);
