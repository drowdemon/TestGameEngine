#include "stdafx.h"
#include "test game engine v14.h"
#include "globals.h"
...
vector<point> astarsearch(problem p, unit *u)
{
	...
	while(true)
	{
		...
		vector <point> add;
		vector <point> moves;
		p.getsuccessors(fringe[minindex].state, add, moves, u);
...
bool calcmove(unit *u, point gowhere)
{
	float dirx=gowhere.x-u->x;
	float diry=gowhere.y-u->y;
	vector<point> temp=allobstacles[u->player][u->index];
	allobstacles[u->player][u->index]=astarsearch(problem(point(gowhere.x,gowhere.y),dirx>0,diry>0),u);
	float difx=0;
	float dify=0;
	for(unsigned int i=0; i<allobstacles[u->player][u->index].size(); i++)
	{
		difx+=allobstacles[u->player][u->index][i].x;
		dify+=allobstacles[u->player][u->index][i].y;
	}
	if(difx!=0 || dify!=0)
	{
		float toaddx=gowhere.x-(u->x+difx);
		float toaddy=gowhere.y-(u->y+dify);
		if(((toaddx+.0001)/abs(toaddx+.0001)==(toaddx/abs(toaddx)) && (toaddx-.0001)/abs(toaddx-.0001)==(toaddx/abs(toaddx))) || ((toaddy+.0001)/abs(toaddy+.0001)==(toaddy/abs(toaddy)) && (toaddy-.0001)/abs(toaddy-.0001)==(toaddy/abs(toaddy)))) //toadd is not approximately 0
			allobstacles[u->player][u->index].push_back(point(toaddx,toaddy));
		for(unsigned int i=0; i<temp.size(); i++)
			allobstacles[u->player][u->index].push_back(temp[i]);
	}
	else
		return false;
	return true;
...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
	...
	LoadString(hInstance, IDC_TESTGAMEENGINEV14, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_TESTGAMEENGINEV14));
	// Main message loop:
	while (GetMessage(&msg, NULL, 0, 0))
	{
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_TESTGAMEENGINEV14));
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_TESTGAMEENGINEV14);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		if(!inf3)
		{
			MessageBox(hWnd, "Unable to open map2.specs", "Error", NULL);
			exit(-235);
		}
		...
		alldisp.push_back(display("Food: ", width/3+5+370, 612+1*18, YOUR_BUILDING, dispresourcesbuilding));
		alldisp.push_back(display("Wood: ", width/3+5+370, 612+2*18, YOUR_BUILDING, dispresourcesbuilding));
		alldisp.push_back(display("Gold: ", width/3+5+370, 612+3*18, YOUR_BUILDING, dispresourcesbuilding));
		alldisp.push_back(display("Stone: ", width/3+5+370, 612+4*18, YOUR_BUILDING, dispresourcesbuilding));
		...
		alldisp.push_back(display("Health: ", width/3+5+370, 612, YOUR_BUILDING, dispbuildinghealth));
		alldisp.push_back(display("", width/3+5+275, 612, YOUR_BUILDING, dispbuildingname));
	...
	case WM_LBUTTONDBLCLK:
	{
		...
		if(map[(int)(mousey/15)][(int)(mousex/15)].uniton==true)
			reg=allunits[0][map[(int)(mousey/15)][(int)(mousex/15)].unitindex]->regimentid;
		if(reg!=-1)
		{
			for(unsigned int i=0; i<selectedunits[0].size(); i++)
	...
	case WM_RBUTTONDOWN:
	{
		...
		p.x/=15;
		p.y/=15;
		if((wParam&MK_SHIFT)==0) //not holding shift	
			move(0, p);
		else //holding shift
			move(0,p,true);
		break;
...
VOID CALLBACK mainTimerProc(HWND hWnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime)
{
	...
	{
		redraw=false;
		/*SelectObject(hdc, hbmBackBM);
		DeleteDC(hdc);
		SelectObject(hdcBuf, hbmBackBM);
		DeleteDC(hdcBuf);*/
	...
	WCHAR text[1];
	for(unsigned int i=(int)topleft.y; i<topleft.y+client.bottom/15+1; i++) //loop through map, print tiles
	{
	...
	for(unsigned int i=(int)topleft.y; i<topleft.y+client.bottom/15+1; i++) //loop through map, to print, only print stuff for units and buildings
	{
		for(unsigned int j=(int)topleft.x; j<topleft.x+client.right/15; j++)
		{
			...
			{	
				...
				if((int)allbuildings[player][index].x==j && (int)allbuildings[player][index].y==i)//print only once
				{
					if(allbuildings[player][index].health>0)//don't print dead things
					{
						...
						g.DrawString(text,allbuildablebuildings[allbuildings[player][index].id].name.size(),&font,Gdiplus::PointF(Gdiplus::REAL((j-topleft.x)*15+2), Gdiplus::REAL((i-topleft.y)*15+2)),&black);
						if(allbuildings[player][index].beingbuilt<0)
						{
							...
							if(allbuildings[player][index].selected==false && player==0)
								g.DrawRectangle(&(p[4]), (Gdiplus::REAL)(j-topleft.x)*15-4, (Gdiplus::REAL)(i-topleft.y)*15, (Gdiplus::REAL)allbuildings[player][index].width*15, (Gdiplus::REAL)allbuildings[player][index].height*15);
			...
			if(allunits[0][generals[0]]->los<dist) //far away - print 'fog of war' //REALLY EXPENSIVE CPU-wise
